...
Alvex.util.isInDocRegForm = function Alvex_isInDocRegForm(id)
{
	// TODO Is this check enough?
	var form = Alvex.util.getFormElement(id);
	// It happens if we are on workflow details page - there is not form in this case
	if( !form )
		return false;
	var idMatches = (form.id.match('alvex-docreg-popup-dialog') != null);
	if(idMatches)
		return true;
	else
		return false;
...
{
	var el = YAHOO.util.Selector.query(".alfresco-header-Title__text")[0];
	return el.title;
};
